        ...
        <a-col :flex="1">
          <a-form :model="formModel">
            <a-row :gutter="16">
              ...
              <a-col :span="8">
                <a-form-item field="name" label="内容体裁">
                  <a-select
              ...
              <a-col :span="8">
                <a-form-item field="name" label="筛选方式">
                  <a-select
      ...
      >
        ...
        <template #contentType="{ record }">
          <a-space>
            ...
            {{ ListColumn.contentType[record.contentType] }}
          </a-space>
        </template>
        <template #filterType="{ record }">
          {{ ListColumn.filterType[record.filterType] }}
        </template>
        <template #status="{ record }">
          <span v-if="record.status === 'offline'" class="circle"></span>
          <span v-else class="circle pass"></span>
          {{ ListColumn.status[record.status] }}
        </template>
        <template #operations>
...
import { reqProjectList } from '@/api/test'
import type { Pagination } from '@/types/global'
import type { PolicyRecord, PolicyParams } from '@/api/test/type'
...
const getProjectList = async (
  ...
  setLoading(true)
  try {
    const { data } = await reqProjectList(params)
